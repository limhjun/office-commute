...
- `service/overtime/HolidayDataUnavailableException`(추정 위치)
...
**포함** (컨벤션 정합 + 회귀 안전성)
- G1 — `employee_id` NOT NULL 제약 (Flyway V4 추가)
- G2 — 등록 응답을 `CommuteRegisterResponse(commuteHistoryId)` 형태로 변경 (POST/PUT 모두 또는 POST만)
- G3 — `POST /commute` 201 Created
- G4 — `CommuteHistoryControllerTest` 신설 (권한·HTTP 시나리오·에러 매핑)
- G5 — `work_start_time` NOT NULL 검토 (연차 레코드 영향 확인 후 결정)

**제외** (범위 또는 정책 결정 필요)
- G6 (야간 근무) — 정책 정립 후 별도 항목으로 처리
- G7 (점심 차감) — 정책 정립 후 별도 항목 또는 ⑧과 묶음
- G8 (관리자 보정) — 별도 신규 기능 항목
- G9 (시간대 매핑) — 코드 변경 없이 검증/문서화로 마무리 가능 (선택)
- G10 — ④에서 처리

### 결정사항 (사용자 확인 완료)

1. **G2 응답** — POST만 `{ commuteHistoryId }`, PUT은 void. → 별도 라운드 (G1~G5)에서 처리.
2. **G5 work_start_time NOT NULL** — 별도 라운드 (G1~G5)에서 처리.
3. **G9 시간대** — **c-3 채택**. `employee.timezone`(직원의 기본 시간대) + `commute_history.work_zone`(레코드별 snapshot) 모두 도입. 본 라운드에서 처리.
4. **본 라운드 범위** — G9(c-3)만 구현. G1~G5는 후속 라운드.

### c-3 적용 범위
- 스키마: `employee.timezone`, `commute_history.work_zone` 컬럼 추가 (Flyway V4, 기본값 `Asia/Seoul`).
- `TimeConfig` + `Clock` 빈 — `Clock.systemUTC()`. 서비스에서 직원 zone으로 `withZone` 후 `ZonedDateTime.now(...)`.
- `Employee.timezone` 필드 + 정적 팩토리 시그니처 확장. 검증: blank 차단(IAE).
- `EmployeeSaveRequest.timezone`(옵셔널, null이면 서비스에서 `Asia/Seoul`로 기본값), `EmployeeFindResponse.timezone`.
- `CommuteHistory.workZone` 필드 + 생성자 확장. 등록 시 직원 zone 사용. 퇴근 시각도 *원래 출근 레코드의 workZone*으로 일관 처리.
- `CommuteHistoryController.registerWorkEndTime` — 외부 시각 인자 제거, 서비스 내부에서 직원 zone 기반 생성.
- `AnnualLeaveService` → `CommuteHistory(employeeId, leaveDate)` 생성 시 직원 zone 전달.
- 픽스처(`Employees`, `EmployeeBuilder`), `data.sql`, `openapi.yml` 일괄 갱신.

### 결정 필요 사항 (사용자 합의 후 확정 — 후속 라운드)
...
3. **G9 — 시간대 매핑 범위**
   - (a) 검증만 수행하고 문서화 (코드 변경 없음).
   - (b) `ZonedDateTime` 매핑 명시화 (JPA `@Convert` 또는 Hibernate 시간대 설정).
   - 권장: (a) — 본 라운드 폭 최소화.
